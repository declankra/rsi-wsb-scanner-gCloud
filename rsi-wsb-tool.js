function sendVariablesToCloudFunction() {
  ...
  function appendDataToSheet(sheet, data, headers) {
  ...
  console.log("Received gSheetsHistoryDF:", JSON.stringify(responseData.gSheetsHistoryDF));
  console.log("Received gSheetsResultsDF:", JSON.stringify(responseData.gSheetsResultsDF));

  // Append gSheetsResultsDF to "3: see results" sheet
  if (responseData.gSheetsResultsDF) {
    appendDataToSheet(resultSheet, responseData.gSheetsResultsDF, result_headers);
  }

  // Append gSheetsHistoryDF to "4: track history" sheet
  if (responseData.gSheetsHistoryDF) {
    appendDataToSheet(historySheet, responseData.gSheetsHistoryDF, history_headers);
  }
