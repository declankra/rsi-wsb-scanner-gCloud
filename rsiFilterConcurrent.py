### instead of fetching data for each symbol individually, yfinance allows for downloading of multiple symbols at once so we make "batch" requests ---- REVERTED -> fetches symbols one at time, but in parallel / concurrently
...
###  Instead of using yf.download, we're now using yf.Ticker(symbol).history() to fetch data. This method is sometimes more reliable for individual symbol lookups.
...
def fetch_and_calculate_rsi(symbol, period, threshold):
    try:
        ...
        # logging.info(f"Calculated RSI for {symbol}: {rsi_last}")
...
def rsiFilter(symbols, rsiPeriod, rsiThreshold):
    ...
    with ThreadPoolExecutor(max_workers=5) as executor:
        futures = [executor.submit(fetch_and_calculate_rsi, symbol, rsiPeriod, rsiThreshold) for symbol in symbols]
